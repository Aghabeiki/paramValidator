/**
 * Created by roten on 11/16/17.
 */

const baseValidator = require('../index');
const PVError = require('../../Error/PVError');
const URLPattern = require('url-pattern');
const ParamValidator = require('../../base/Validator');
const _ = require('lodash');
const urlPatternsConfig = {segmentNameCharset: 'a-zA-Z0-9_-'};
class SailsJSParamValidator extends baseValidator {
    ...
    validator(req, res, next) {
        try {
            ...
            if (req.method.toLowerCase() === 'get') {
                ...
                let queryString = Object.keys(that.routes)
                    .filter(route => {
                        "use strict";
                        return (route.toLowerCase().startsWith('get') &&
                        ((new URLPattern(route.toLowerCase().replace('get ', ''), urlPatternsConfig).match(targetURL.url.toLowerCase()) !== null)))
                    })
                    .filter((finedURL, index, array) => {
                        "use strict";
                        return (array.length === 1 || (finedURL.toLowerCase().replace('get', '').replaceAll(' ', '') === (targetURL.url[targetURL.url.length - 1] === '/' ? targetURL.url.substring(0, url.length - 1).toLowerCase() : targetURL.url.toLowerCase())))
                    })
                    .map(route => {
                        "use strict";
                        let bank = {};
                        ...
                        bank[tmp] = (new URLPattern(tmp, urlPatternsConfig)).match(targetURL.url)
                ...
                if (queryString.length == 0) {
                    that.logger.error('the route is not found in bank url query ');
                    return res.json(400, {
                        error_code: 400,
                        error_message: 'Invalid param validator configuration',
                        error_detail: 'the route not found in the bank, check ur requested url,or call the administrator'
                    });
                }
                ...
                let urlPattern = new URLPattern(paramValidator.targetURL, urlPatternsConfig);
                let params = req.body;
                let pathParams = urlPattern.match(req.url) || {};
                Object.keys(pathParams).forEach(key => {
                    "use strict";
                    params[key] = pathParams[key];
                });
                params.headerParams = req.headers
                paramValidator.validate(params, paramValidatorCB);
            }
...
module.exports = SailsJSParamValidator;