    ...
    constructor(method, url, bankPath, scriptPath) {
        ...
        try {
            let targetURL = Object.keys(bank).filter(URL => {
                if (bank[URL][method.toLowerCase()] !== undefined) {
                    let URLPattern = require('url-pattern')
                    let urlPattern = new URLPattern(URL, urlPatternsConfig);
                    return urlPattern.match(url.toLowerCase()) !== null;
                }
                else {
                    return false;
                }
            ...
            if (targetURL.length === 1) {
                ...
                let trimmedURL = url[url.length - 1] === '/' ? url.substring(0, url.length - 1) : url;
                let tmpURLs = targetURL.filter(finedURL => {
                    return finedURL.toLowerCase() === trimmedURL.toLowerCase()
                })
                if (tmpURLs.length === 1) {
                    _target.set(ValidatorHandler, bank[tmpURLs[0]][method.toLowerCase()]);
                    _targetURL.set(ValidatorHandler, tmpURLs[0]);
                }
                else {
                    let targetUrl = '';
                    if (url[url.length - 1] == '/') {
